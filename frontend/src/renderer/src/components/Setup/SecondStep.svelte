<script lang="ts">
  ...
  try {
    ...
    if (adminPassword.length < 4) {
      throw { message: "Beheerderscode moet minimaal 4 tekens lang zijn", details: null };
    }
  ...
  <div class="mx-auto pt-10 w-full max-w-[800px] max-h-200px flex flex-col gap-10 overflow-y-auto">
    <div>
  ...
  <Footer class="absolute bottom-0 start-0 z-20 w-full p-4 bg-white border-t border-gray-200 shadow flex items-center justify-end">
    <Button
      type="submit"
      class="gap-2 px-2"
      disabled={isLoading}
