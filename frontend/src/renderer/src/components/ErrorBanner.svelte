<script lang="ts">
  ...
  import { CloseCircleOutline, ChevronDownOutline } from 'flowbite-svelte-icons';
...
{#if error}
  ...
  <Banner position="absolute" color="gray">
    ...
    <span class="flex items-center text-sm font-medium">
      <span class="inline-flex p-1 me-3 bg-red-100 rounded-full dark:bg-red-900">
    ...
    {#if error.details && showDetails}
      <span class="ml-8 text-xs mt-1 text-red-600 dark:text-red-300" transition:fade>
        {error.details}
      </span>
    {/if}
    </p>
